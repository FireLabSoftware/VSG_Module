...
def vLog(*note,LogFileName=vDefaultLogFileName,vFileOut=True,vConsoleOut=True):  ## vsilent avoids terminal, vstealth avoids writing to file
    if note and (note[-1]=='vsilent'): ## two ways to control output.  Last item 'vsilent' avoids console out, 'vstealth' avoidds file out
        note = note[:-1]
        vSilent = True
    else:
        vSilent = False
    if note and (note[-1]=='vstealth'):
        note = note[:-1]
        vStealth = True
    else:
        vStealth = False
    ...
    if vFileOut and not(vStealth):
        ...
        LogFile.close()
    if vConsoleOut and not(vSilent):
...
def vLogOpeningFile(FileID,kw):
    ...
    vConsoleOut=True
    if 'vFileOut' in kw:
        vFileOut = kw['vFileOut']
    if 'vConsole' in kw:
        vConsole = kw['vConsole']
    kwlist = ''
    for k in kw:
        kwlist+=k+'='+str(kw[k])+' '
    vLog("opening file "+vFileInfo(FileID)+'  '+kwlist,vFileOut=vFileOut,vConsoleOut=vConsoleOut)
def vLogClosingFile(FileID,vFileOut=True,vConsoleOut=True):
    vLog("closing file "+vFileInfo(FileID),vFileOut=vFileOut,vConsoleOut=vConsoleOut)
def vLogRunningFile(FileID,vFileOut=True,vConsoleOut=True):
    vLog("running file"+vFileInfo(FileID),vFileOut=vFileOut,vConsoleOut=vConsoleOut)
def vOpen(FileName,**kw):
    vLogOpeningFile(FileName,kw)
    if not('mode') in kw:
        kw['mode'] = 'rt'
    if FileName.endswith('.gz') or FileName.endswith('.gzip'):
        F0=gzip.open(FileName,mode=kw['mode'])
    elif FileName.endswith('.zip'):
        if kw['mode'].startswith('w'):
            TextFileName = FileName[::-1].replace('piz.','txt.',1)[::-1]
            F0 = open(TextFileName, mode=kw['mode']) ## we don't actually write zip files here... redirect to a standard text file 
        else:
            ZipFile1 = zipfile.ZipFile(FileName)
            FileList = []
            for zfn1 in ZipFile1.namelist():
                if zfn1[0]!='_':
                    FileList.extend(ZipFile1.open(zfn1,mode=kw['mode']))
            F0 = chain(FileList)
    else:
        F0=open(FileName,mode=kw['mode'])
    return F0
def vClose(Fileobject,vFileOut=True, vConsoleOut=True):
    vLogClosingFile(Fileobject,vFileOut=vFileOut,vConsoleOut=vConsoleOut)
    Fileobject.close()
def vhelp():
    print()
    print('* * * * * * * * * * * * * * * * * * * * * * * * * * * * * * ')
